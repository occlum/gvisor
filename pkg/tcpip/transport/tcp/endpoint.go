...
// TODO: Notify all the waiters when the send buffer size changes.
func (e *endpoint) OnSendBufferSizeOptionSet(sendBufferSize int64) (int64, bool) {
	...
	if v < math.MaxInt32/tcpip.PacketOverheadFactor {
		v *= tcpip.PacketOverheadFactor
		if v < int64(ss.Min) {
			v = int64(ss.Min)
		}
	} else {
